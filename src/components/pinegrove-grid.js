...
import {
  pinegroveGridDesktop,
  pinegroveGridMobile,
} from '../helpers/pinegroveGridBuilder'
...
export default ({ links, desktopLocations, mobileLocations }) => {
  ...
  return (
    <div
      className={isMobileDevice ? 'pinegrove-grid-mobile' : 'pinegrove-grid'}
    >
      {isMobileDevice
        ? pinegroveGridMobile.map(element => {
            if (element.link) {
              if (element.link.location.includes('http')) {
                return (
                  <a
                    className={`pinegrove-grid-link ${element.type}`}
                    href={element.link.location}
                    key={element.link.location}
                  >
                    <img src={element.image} />
                    <h3 className="pinegrove-grid-link-text">
                      {element.link.text}
                    </h3>
                  </a>
                )
